...
// File @openzeppelin/contracts/utils/escrow/Escrow.sol@v4.8.3
...
// File @openzeppelin/contracts/security/PullPayment.sol@v4.8.3
...
// File src/IFeeder.sol


pragma solidity ^0.8.17;

interface IFeeder {
    function increaseFertility(uint256 tokenId, uint8 amount) external payable;
}
...
/// @title CryptoChronic Feeder
contract Feeder is IFeeder, Ownable, Pausable, AccessControl, PullPayment {
    ...
    // state variables
    uint256 public fertilityPrice = 0.003 ether;

    // events
    event ChangedFertilityPrice(uint256 oldValue, uint256 newValue);

    /// constructor
    constructor(address[] memory finance, address[] memory admin) {
        _pause();
    ...
    function setFertilityPrice(uint256 value) external  whenPaused {
        require(hasRole(FINANCE_ROLE, msg.sender), "Caller is not a finance");
        uint256 oldValue = fertilityPrice;
        fertilityPrice = value;
        emit ChangedFertilityPrice(oldValue, value);
    }
    ...
    {
        require(
            _msgSender() == cc.ownerOf(tokenId),
            "Sender is not the owner of tokenId"
        );
        require(
            (msg.value >= (fertilityPrice * amount)),
            "Amount is less than fertilityPrice"
        );
        cc.increaseFertility(tokenId, amount);
    }
